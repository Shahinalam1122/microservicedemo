...
namespace Discount.Grpc.Repository
{
    public class CouponRepository : ICouponRepository
    {
        ...
        public async Task<Coupon> GetDiscount(string produtId)
        {
            ...
            var coupon = await connection.QueryFirstOrDefaultAsync<Coupon>("SELECT * FROM Coupon WHERE ProductId=@ProductId", new { ProductId = produtId });
            if(coupon == null)
            {
        ...
        public async Task<bool> DeleteDiscount(string produtId)
        {
            var connection = new NpgsqlConnection(_configuration.GetConnectionString("DiscountDB"));
            var affected = await connection.ExecuteAsync("DELETE FROM Coupon WHERE ProductId=@ProductId", new { ProductId=produtId});
            if(affected > 0)
            {
                return true;
            }
